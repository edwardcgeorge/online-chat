...
//
mongoose.createConnection('mongodb://root:wqXAExX6Z44o@/opt/bitnami/mongodb/tmp/mongodb-27017.sock/chatroom');
...
var messagesData = mongoose.model('message', messageSchema);
...
io.sockets.on('connection', function(socket){
    ...
    socket.on('message', function(message, id){
        ...
        insertMessage(roomName, socket.nickname, message, time);
...
app.get('/new', function(request, response){
  ...
  var time = new Date().getTime();
    var newRoom = new roomData({ room: newName, createTime: time});
...
app.get('/getRoom', function(request, response){
    var messages;
    // roomData.remove().exec();
    roomData.find().sort({'createTime': -1}).exec(function (err, res) {
        if(res != null) {
            console.log(res);
            response.send(res);
        }
...
function getMessage(roomName, callback){
    messagesData.find({'room': roomName}).exec(function (err, res) {
...
function insertMessage(roomName, nickname, message, time){
    // console.log(time);
    var message = new messagesData({ room: roomName, nickname: nickname, body: message, time: time});
